    ...
    def run(self, run_id: str | None = None, stop_after: str = "reports") -> dict[str, Any]:
        ...
        try:
            ...
            if self.output_dir:
                ...
                tiered = write_tiered_reports(
                    run_id, manifest.get("started_at", ""), self.output_dir,
                    table_outputs, all_reviews,
                )
                self._event("info", "tiered_reports_written", files=list(tiered.keys()))
            if self.connectors:
