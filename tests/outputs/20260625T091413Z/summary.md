...
**Failures:**
- `row_count`: The data quality check failed because the row count for `sample_dim_brand_scd2` did not match between the source (30 rows) and the target (31 rows). Additionally, the date coverage check failed becaus
...
- `distribution__brand_name`: The data quality check failed because the distribution of `brand_name` values between the source and target datasets does not match. Specifically, there are 12 mismatches found in the top 20 values, i
...
- `key_buckets`: The key bucket check failed because the target dataset contains a key bucket ('b7') that is missing in the source dataset. Specifically, the comparison found 1 mismatch where 'b7' was present in the t
...
- `mapped_rows_when_small`: The data quality check failed because the target table (`sample_dim_brand_scd2`) contained one extra key hash that was not present in the source table. Specifically, the `extra_key_hashes` list shows 
...
- [measure] plan_quantity: confidence 80%
